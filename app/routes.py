...
def subscribe():
    """プッシュ通知の購読登録"""
    from app.push_notifications import save_subscription
    
    subscription_info = request.json
    if not subscription_info or not subscription_info.get('endpoint'):
        return jsonify({'success': False, 'error': '有効なサブスクリプション情報が必要です'}), 400
    
    success = save_subscription(subscription_info)
    if success:
        return jsonify({'success': True}), 201
    else:
        return jsonify({'success': False, 'error': 'サブスクリプションの保存に失敗しました'}), 500

@bp.route('/api/unsubscribe', methods=['POST'])
def unsubscribe():
    """プッシュ通知の購読解除"""
    from app.push_notifications import remove_subscription
    
    subscription_info = request.json
    if not subscription_info or not subscription_info.get('endpoint'):
        return jsonify({'success': False, 'error': 'エンドポイント情報が必要です'}), 400
    
    success = remove_subscription(subscription_info.get('endpoint'))
    if success:
        return jsonify({'success': True}), 200
    else:
        return jsonify({'success': False, 'error': 'サブスクリプションの削除に失敗しました'}), 500

@bp.route('/api/send-notification', methods=['POST'])
def send_notification():
    """テスト用：単一の通知を送信"""
    from app.push_notifications import broadcast_notification
...
@bp.route('/api/astronomical-events', methods=['POST'])
def trigger_astronomical_events():
    """天文イベント通知をトリガーする（管理者用）"""
